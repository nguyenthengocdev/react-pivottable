...
| `PivotData`    | `aggregatorNames` <br /> array of strings        | `[]`                          | optional list of aggregator keys to compute simultaneously; if omitted only `aggregatorName` is evaluated. The value of `aggregatorName` is always included in this list even if it is not provided explicitly. Renderers can then choose how to present each aggregator (the stock table renderer treats them as an additional “Values” row attribute).                                                                                                                                                                                                                                                   |
| `PivotData`    | `aggregations` <br /> array of objects           | `[]`                          | advanced configuration for measures. Each object can contain `key`, `aggregatorName`, `vals`, and an optional `label`. Use this to compute multiple aggregator/value combinations simultaneously (e.g. `[{aggregatorName: 'Count', vals: ['Party Size']}, {aggregatorName: 'Average', vals: ['Tip']}]`). When supplied, this supersedes `aggregatorNames`/`vals`; the first aggregation is treated as the “primary” one for backwards compatibility with renderers.                                                                                                                                                |
